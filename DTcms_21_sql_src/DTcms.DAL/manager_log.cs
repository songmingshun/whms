...
namespace DTcms.DAL
{
    ...
    public partial class manager_log
    {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from gm_dt_sysuser_login_log");
        ...
        public int GetCount(string strWhere)
        {
            ...
            strSql.Append(" from gm_dt_sysuser_login_log ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into gm_dt_sysuser_login_log(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,user_id,user_name,action_type,note,login_ip,login_time from gm_dt_sysuser_login_log ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        public Model.manager_log GetModel(string user_name, int top_num, string action_type)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select top 1 id from (select top " + top_num + " id from gm_dt_sysuser_login_log where user_name=@user_name and action_type=@action_type order by id desc) as T ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM gm_dt_sysuser_login_log");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }

...
namespace DTcms.DAL
{
    ...
    public partial class manager_role
    {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from gm_dt_sysuser_role");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 role_name from gm_dt_sysuser_role");
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update gm_dt_sysuser_role set ");
            strSql.Append("role_name=@role_name,");
            strSql.Append("role_type=@role_type,");
            strSql.Append("role_value=@role_value");
            strSql.Append(" where id=@id ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4),
					new SqlParameter("@role_name", SqlDbType.NVarChar,100),
            ...
            parameters[3].Value = model.role_value;
            ...
            strSql2.Append("delete from gm_dt_sysuser_role_value where role_id=@role_id ");
            ...
            if (model.manager_role_values != null)
            {
                ...
                foreach (Model.manager_role_value models in model.manager_role_values)
                {
                    ...
                    if (models.id == 0)
                    {
                        strSql3.Append("insert into gm_dt_sysuser_role_value(");
        ...
        public bool Delete(int id)
        {
            List<CommandInfo> sqllist = new List<CommandInfo>();
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from gm_dt_sysuser_role ");
            strSql.Append(" where id=@id ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
            ...
            sqllist.Add(cmd);
            StringBuilder strSql2 = new StringBuilder();
            strSql2.Append("delete from gm_dt_sysuser_role_value ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,role_name,role_type,role_value from gm_dt_sysuser_role ");
            strSql.Append(" where id=@id ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["role_value"].ToString() != "")
                {
                    model.role_value = int.Parse(ds.Tables[0].Rows[0]["role_value"].ToString());
                }
                #endregion  父表信息end

                #region  子表信息
                StringBuilder strSql2 = new StringBuilder();
                strSql2.Append("select id,role_id,channel_name,channel_id,action_type from gm_dt_sysuser_role_value ");
                strSql2.Append(" where role_id=@role_id ");
                SqlParameter[] parameters2 = {
					new SqlParameter("@role_id", SqlDbType.Int,4)};
                parameters2[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,role_name,role_type,role_value ");
            strSql.Append(" FROM gm_dt_sysuser_role ");
            if (strWhere.Trim() != "")
            {

    ...
    $(function () {
        $("#ddlRoleType").change(function () {
            if ($(this).find("option:selected").attr("value") == 1) {
                $(".cball,.config,.workinghour,.joborder,.customer,.item_view,.item_add,.item_edit,.item_delete").attr("disabled", "disabled");
                //默认权值为0
                //$("#txtRoleValue").val("0");
                //$("#txtRoleValue").attr("disabled", "disabled");
            } else {
                $(".cball,.config,.workinghour,.joborder,.customer,.item_view,.item_add,.item_edit,.item_delete").attr("disabled", "");
            }
        });
    });
    //选中相关的checkbox
...
<div id="contentTab">
    ...
    <div class="tab_con" style="display:block;">
        <table class="form_table">
            ...
            <tr>
                <th>权值：</th>
                <td><asp:TextBox ID="txtRoleValue" runat="server" CssClass="txtInput normal small required number" maxlength="100" ></asp:TextBox><label>*请输入整型数字</label></td>
            </tr>
            <tr>
                <th>权限：</th>
                <td>
...
<%--            <%if (siteConfig.memberstatus == 1)
            ...
            <tr>
                <th></th>
                <td>
                    <label style="margin:0;"><input type="checkbox" class="cball" onclick="SelectedItems(this, 'workinghour');" /><b>工时设置</b></label>
                </td>
            </tr>
            <tr>
                <th></th>
                <td class="item_list">
                    <ul>
                        ...
                        <li>
                            <asp:HiddenField ID="hidId" Value="0" runat="server" />
                            <asp:HiddenField ID="hidName" Value='<%#Eval("name")%>' runat="server" />
                            <label><input type="checkbox" runat="server" id="cblNavName" value='<%#Eval("value")%>' class="workinghour" /><%#Eval("text")%></label>
                        </li>
                        </ItemTemplate>
                        </asp:Repeater>
                    </ul>
            ...
            <tr>
                <th></th>
                <td>
                    <label style="margin:0;"><input type="checkbox" class="cball" onclick="SelectedItems(this, 'joborder');" /><b>工单设置</b></label>
                </td>
            </tr>
            <tr>
                <th></th>
                <td class="item_list">
                    <ul>
                        ...
                        <li>
                            <asp:HiddenField ID="hidId" Value="0" runat="server" />
                            <asp:HiddenField ID="hidName" Value='<%#Eval("name")%>' runat="server" />
                            <label><input type="checkbox" runat="server" id="cblNavName" value='<%#Eval("value")%>' class="joborder" /><%#Eval("text")%></label>
                        </li>
                        </ItemTemplate>
                        </asp:Repeater>
                    </ul>
            ...
            <tr>
                <th></th>
                <td>
                    <label style="margin:0;"><input type="checkbox" class="cball" onclick="SelectedItems(this, 'customer');" /><b>客户设置</b></label>
                </td>
            </tr>
            <tr>
                <th></th>
                <td class="item_list">
                    <ul>
                        ...
                        <li>
                            <asp:HiddenField ID="hidId" Value="0" runat="server" />
                            <asp:HiddenField ID="hidName" Value='<%#Eval("name")%>' runat="server" />
                            <label><input type="checkbox" runat="server" id="cblNavName" value='<%#Eval("value")%>' class="customer" /><%#Eval("text")%></label>
                        </li>
                        </ItemTemplate>
                        </asp:Repeater>
                    </ul>
